...
static constexpr const char* EXCHANGE_ADDR    = "0xe111180000d2663c0091e4f400237545b87b996b";
...
static constexpr int ROT[25] = {
     0,  1, 62, 28, 27, 36, 44,  6, 55, 20,
     3, 10, 43, 25, 39, 41, 45, 15, 21,  8,
    18,  2, 61, 56, 14
};
static constexpr int PI[25] = {
     0, 10,  7, 11, 17, 18,  3,  5, 16,  8,
...
static std::string derive_address(const std::vector<uint8_t>& privkey) {
    ...
    size_t plen = EC_POINT_point2oct(grp, pub, POINT_CONVERSION_UNCOMPRESSED, nullptr, 0, nullptr);
    std::vector<uint8_t> pubbuf(plen);
    EC_POINT_point2oct(grp, pub, POINT_CONVERSION_UNCOMPRESSED, pubbuf.data(), plen, nullptr);
...
static std::string sign_digest(const std::array<uint8_t,32>& digest,
    ...
    std::string hex_sig = "0x" + to_hex(r_bytes.data(), 32)
                               + to_hex(s_bytes.data(), 32);
    // Append v byte
...
static std::array<uint8_t,32> order_type_hash() {
    static const std::string t =
        ...
        "uint256 signatureType,uint256 timestamp,bytes32 metadata,bytes32 builder)";
    return keccak256(t);
}

static std::array<uint8_t,32> compute_domain_separator(const std::string& maker_addr) {
...
static void compute_amounts(const std::string& side, double price, double size,
                            std::string& maker_amount, std::string& taker_amount) {
...
public:
    Executor() {
        ...
        if (!pk || !ak || !pp)
            throw std::runtime_error("Missing env: POLY_PRIVATE_KEY, POLY_API_KEY, POLY_PASSPHRASE");
    ...
    json process_signal(const json& sig) {
        try {
            ...
            if (res != CURLE_OK) {
                return {{"status", "error"}, {"order_id", ""}, {"detail", curl_easy_strerror(res)}};
            }

            // Parse API response
            json api_resp;
            try { api_resp = json::parse(response); } catch (...) {
                return {{"status", "error"}, {"order_id", ""}, {"detail", "invalid JSON response"},
                        {"http_code", http_code}, {"raw", response}};
            }

            std::string order_id = api_resp.value("orderID", api_resp.value("order_id", ""));
            ...
            if (http_code >= 200 && http_code < 300 && !order_id.empty()) {
                ...
                return {{"status", "error"}, {"order_id", order_id},
                        {"http_code", http_code}, {"detail", response}};
            }
